...
def load_images_tuple(folder_or_list, size, square_ok=False, verbose=True):
    """ open and convert all images in a list or folder to proper input format for DUSt3R
        size: int (short/long side) or tuple (W, H)
    """
    ...
    for path in folder_content:
        ...
        if isinstance(size, int):
            ...
            img = img.resize((W_target, H_target), PIL.Image.BICUBIC)

        W, H = img.size
        cx, cy = W//2, H//2
        if isinstance(size, int) and size == 224:
            half = min(cx, cy)
